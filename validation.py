...
class DataValidation:
    ...
    def connect_spark(self) -> SparkSession:
        try:
            ...
            conf.set("spark.sql.catalog.iceberg.type", "rest")  # Explicitly set type to REST
            conf.set("spark.sql.catalog.iceberg.catalog-impl", "org.apache.iceberg.rest.RESTCatalog")
            conf.set("spark.sql.catalog.iceberg.uri", f"http://iceberg-rest-catalog.{self.iceberg_config['external_ip']}.nip.io")
            conf.set("spark.sql.catalog.iceberg.warehouse", self.iceberg_config['iceberg_warehouse_path'])
            conf.set("spark.sql.warehouse.dir", self.iceberg_config['iceberg_warehouse_path'])

            # # S3/Ceph configuration
            # conf.set("spark.hadoop.fs.s3a.endpoint", f"http://ceph.{self.iceberg_config['external_ip']}.nip.io")
            # conf.set("spark.hadoop.fs.s3a.access.key", self.iceberg_config['s3_access_key'])
            # conf.set("spark.hadoop.fs.s3a.secret.key", self.iceberg_config['s3_secret_key'])
            # conf.set("spark.hadoop.fs.s3a.path.style.access", "true")
            # conf.set("spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem")
            
            # # Add: Set default file system explicitly
            # conf.set("spark.hadoop.fs.defaultFS", self.iceberg_config['iceberg_warehouse_path'])
